      ...
      <div className='flex items-center gap-4 '>
        ...
        <div className={`md:hidden ${showMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            ...
            <NavLink onClick={() => setShowMenu(false)} to='/products' ><p className='px-4 py-2 rounded full inline-block'>OUR PRODUCTS</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/about' ><p className='px-4 py-2 rounded full inline-block'>ABOUT</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/contact' ><p className='px-4 py-2 rounded full inline-block'>CONTACT</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/blogs' ><p className='px-4 py-2 rounded full inline-block'>BLOGS</p></NavLink>
            
            {/* Add Create account button for mobile view */}
            {!token && (
              <button 
                onClick={() => {
                  setShowMenu(false)
